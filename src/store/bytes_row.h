...
using Value = std::variant<std::monostate, int64_t, uint64_t, float,
                           std::string, bool, std::vector<int64_t>,
...
class Schema {
 public:
  ...
  const std::vector<FieldMeta>& get_field_order() const {
...
class BytesRow {
 public:
  ...
  std::string serialize_template(const RowT& row,
    ...
    auto get_default_list_int64 =
        [](const Value& val) -> const std::vector<int64_t>* {
    ...
    auto get_default_list_float32 =
        [](const Value& val) -> const std::vector<float>* {
    ...
    auto get_default_list_string =
        [](const Value& val) -> const std::vector<std::string>* {
    ...
    auto get_list_string_content_len =
    ...
    auto write_list_int64_value = [](const std::vector<int64_t>& vec,
                                     char* dest) {
    ...
    auto write_list_float32_value = [](const std::vector<float>& vec,
                                       char* dest) {
      uint16_t len = static_cast<uint16_t>(vec.size());
      std::memcpy(dest, &len, 2);
      if (len > 0) {
    ...
    auto write_list_string_value = [](const std::vector<std::string>& vec,
                                      char* dest) {
      uint16_t len = static_cast<uint16_t>(vec.size());
      std::memcpy(dest, &len, 2);
      char* cur = dest + 2;
    ...
    for (size_t i = 0; i < field_order.size(); ++i) {
      ...
      switch (meta.data_type) {
        ...
        case FieldType::LIST_STRING: {
          ...
          // length if we compute it that way Or we compute it here: 2 + (2 *
          // list_len) + total_string_bytes
      ...
      switch (meta.data_type) {
        ...
        case FieldType::LIST_STRING: {
          ...
          if (meta.data_type == FieldType::STRING) {
            // Logic handled by accessor to write directly? Or return
            // string_view/string? Accessor returning string creates copy.
            // Better: accessor.write_string(row, i, var_ptr)
            if (has_val) {
              accessor.write_string(row, i, var_ptr);
            } else if (const auto* def =
                           get_default_string(meta.default_value)) {
